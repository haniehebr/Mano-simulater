...
{
    ui->setupUi(this);
    setWindowTitle("Mano Basic Computer Simulator");
    ui->editor->setFocus();
    on_reset_btn_clicked();
    connect(ui->Save_button,SIGNAL(clicked()),this,SLOT(on_savebtn_clicked()));
    connect(ui->run_button,SIGNAL(clicked()),this,SLOT(on_run_btn_clicked()));
    connect(ui->open_button,SIGNAL(clicked()),this,SLOT(on_openbtn_clicked()));
    connect(ui->compile_button,SIGNAL(clicked()),this,SLOT(on_compile_btn_clicked()));
    connect(ui->next_button,SIGNAL(clicked()),this,SLOT(on_next_btn_clicked()));
    connect(ui->reset_button,SIGNAL(clicked()),this,SLOT(on_reset_btn_clicked()));
    connect(ui->new_button,SIGNAL(clicked()),this,SLOT(on_newbtn_clicked()));
    connect(ui->actionAbout_Mano_Simulator,SIGNAL(triggered()),this,SLOT(on_actionAbout_Mano_simulator_triggered()));
    ...
    connect(ui->actionNew,SIGNAL(triggered()),this,SLOT(on_action_New_triggered()));
    connect(ui->actionExit,SIGNAL(triggered()),this,SLOT(on_action_Exit_triggered()));
    connect(ui->actionCompile_program,SIGNAL(triggered()),this,SLOT(on_actioncompile_program_triggered()));
    connect(ui->actionRun_program,SIGNAL(triggered()),this,SLOT(on_actionRun_Program_triggered()));
    connect(ui->actionNext_step,SIGNAL(triggered()),this,SLOT(on_actionnext_step_triggered()));
    connect(ui->actionReset_code,SIGNAL(triggered()),this,SLOT(on_actionreset_code_triggered()));
...
{
    QMessageBox::warning(this, tr("Mano Basic Computer Simulator"),
...
void MainWindow::on_compile_btn_clicked()
{
    ...
    for(int i=0;i<tcommmands;i++)
    {
        if(lc>4096)
        {
            ui->console->setText("ERROR:\nOPPS RAM Overfloweded!\n");
            compiled=0;
            break;
        }
...
void MainWindow::on_next_btn_clicked()
{
    ...
    else
    {
        ...
        ui->console->setText("");
                ...
                SC=clk;
                    QStringList riz = commands.at(lineStep).split(' ',Qt::KeepEmptyParts);
                    if(riz.at(0)=="//" || riz.at(0)[0]=='/')
                    {
                       lineStep++;
                    }
                    ...
                    riz = commands.at(lineStep).split(' ',Qt::KeepEmptyParts);
                    ...
                    else if(riz.at(0)=="HLT")
                    {
                        ...
                        ui->console->setText("Congratulation!\nProgram run successfully");
                        if(printing)
                        {
                            printReg();
                            printTable();
...
{
    QMessageBox msgBox;
    msgBox.setText("Mano Basic Computer Simulater.\nThis program writed using C++ & QT framework:)");
    msgBox.exec();
}
